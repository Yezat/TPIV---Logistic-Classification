...
class RandomKitchenSinkDataModel(DataModel):
    def __init__(self, student_dimension, teacher_dimension, logger, source_pickle_path = "../", delete_existing = False):  
        ...
        logger.info(f"Let that Random Kitchen Sink in, {self.kitchen_kind.name}")
        ...
        if os.path.isfile(source_pickle) and not delete_existing:
            ...
            # ------------- The Student only Kitchen Sink ---------------- 
                ...
                # spec_Omega0 = np.array([k for k in range(1,self.p+1) ])/np.sqrt(self.p)
                spec_Omega0 = np.ones(self.p)
                self.Omega=np.diag(spec_Omega0)
                self.Phi = self.Omega
                self.Psi = self.Omega

                self.theta = np.sqrt(np.array([1/(k+1)**((1+alph*(2*r-1))) for k in range(self.p)]))
                self.theta = np.ones(self.p)
    ...
    def get_data(self, n, tau):
        if self.kitchen_kind == KitchenKind.StudentOnly:
            ...
            u = c
            y = np.sign(u @ the + tau * np.random.normal(0,1,(n,))) 
            X = u
            X_test = np.random.normal(0,1,(100000,self.d)) / np.sqrt(self.d)
            y_test = np.sign(X_test @ the + tau * np.random.normal(0,1,(100000,)))
            return X, y, X_test, y_test, the
            ...
            # let's sample a multivariate normal with zero mean and self.Omega covariance (both scipy and np are extremely slow in mps on my machine) depends on the method I believe.. and maybe some locking issues
            X = np.random.default_rng().multivariate_normal(np.zeros(self.p), self.Psi, n, method="cholesky")  
            # X = np.random.default_rng().multivariate_normal(np.zeros(self.p), self.Psi, n, method="cholesky")  / np.sqrt(np.trace(self.Psi))
            
            y = np.sign(X @ self.theta / np.sqrt(self.p) + tau * np.random.normal(0,1,(n,)))
            
            X_test = np.random.default_rng().multivariate_normal(np.zeros(self.p), self.Psi, 10000, method="cholesky") 
            # X_test = np.random.default_rng().multivariate_normal(np.zeros(self.p), self.Psi, 10000, method="cholesky") / np.sqrt(np.trace(self.Psi))
            y_test = np.sign(X_test @ self.theta / np.sqrt(self.p) + tau * np.random.normal(0,1,(10000,)))
...
if __name__ == "__main__":
    ...
    X, y, X_test, y_test, the = model.get_data(10000, 0.0)
    # average X over all samples
    print(np.linalg.norm(np.mean(X, axis=0)))
    print(np.mean((X@model.theta)**2))
