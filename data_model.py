...
class AbstractDataModel(ABC):
    ...
    def __init__(
        ...
        self.logger.info(
            f"Initializing data model of type {self.model_type.name} with d={d}"
        )
        ...
        self.source_pickle = (
            f"{source_pickle_path}data/data_model_{self.model_type.name}_{d}.pkl"
        )
        if name != "":
        ...
        if os.path.isfile(self.source_pickle) and not delete_existing:
            ...
            if delete_existing:
                self.logger.info(
                    "delete_existing is set to True, deleting existing pickle"
                )
    ...
    def _finish_initialization(self):
        ...
        assumption_1 = (
            self.Sigma_x - self.Sigma_x.T @ np.linalg.inv(self.Sigma_x) @ self.Sigma_x
        )
        ...
        self.logger.info(
            f"Pre-Ratio of first to last eigenvalue of Sigma_x: {self.spec_Sigma_x[0] / self.spec_Sigma_x[-1]}"
        )
        ...
        if self.normalize_matrices:
            ...
            self.Sigma_upsilon = (
                self.Sigma_upsilon / np.trace(self.Sigma_upsilon) * self.d
            )
        ...
        # compute and log the ratio of the first eigenvalue to the last eigenvalue
        self.logger.info(
            f"Ratio of first to last eigenvalue of Sigma_x: {self.spec_Sigma_x[0] / self.spec_Sigma_x[-1]}"
        )
        ...
        self.logger.info(
            f"Sigma_x value counts: {np.unique(self.Sigma_x, return_counts=True)}"
        )
        self.logger.info(
            f"Sigma_w value counts: {np.unique(self.Sigma_w, return_counts=True)}"
        )
        self.logger.info(
            f"Sigma_delta value counts: {np.unique(self.Sigma_delta, return_counts=True)}"
        )
        self.logger.info(
            f"Sigma_upsilon value counts: {np.unique(self.Sigma_upsilon, return_counts=True)}"
        )
        ...
        # Compute FTerm
        self.FTerm = (
            self.Sigma_x.T * self.Sigma_theta * self.Sigma_upsilon
            + self.Sigma_upsilon.T * self.Sigma_theta * self.Sigma_x
        )
    ...
    def store_self_to_pickle(self):
        if self.name != "":
            ...
            if os.path.isfile(self.source_pickle):
                raise Exception(
                    f"File {self.source_pickle} already exists. Please delete it or change the name of the model"
                )
...
class VanillaGaussianDataModel(AbstractDataModel):
    ...
    def generate_data(self, n, tau) -> DataSet:
        ...
        X = np.random.default_rng().multivariate_normal(
            np.zeros(self.d), self.Sigma_x, n, method="cholesky"
        )
        ...
        X_test = np.random.default_rng().multivariate_normal(
            np.zeros(self.d), self.Sigma_x, 100000, method="cholesky"
        )
        y_test = np.sign(
            X_test @ the / np.sqrt(self.d) + tau * np.random.normal(0, 1, (100000,))
        )
...
class KFeaturesModel(AbstractDataModel):
    def __init__(
        ...
        source_pickle_path="../",
        Sigma_w_content=None,
        Sigma_delta_content=None,
        Sigma_upsilon_content=None,
        name="",
        description="",
        feature_ratios=None,
        features_x=None,
        ...
        if not self.loaded_from_pickle:
            ...
            for i in range(k):
                theta[sum(feature_sizes[:i]) : sum(feature_sizes[: i + 1])] = (
                    features_theta[i]
                )
            ...
            for i in range(k):
                sigma_w[sum(feature_sizes[:i]) : sum(feature_sizes[: i + 1])] = (
                    Sigma_w_content[i]
                )
                sigma_delta[sum(feature_sizes[:i]) : sum(feature_sizes[: i + 1])] = (
                    Sigma_delta_content[i]
                )
                sigma_upsilon[sum(feature_sizes[:i]) : sum(feature_sizes[: i + 1])] = (
                    Sigma_upsilon_content[i]
                )
            ...
            if (
                ...
                if process_sigma_type == SigmaDeltaProcessType.ComputeTeacherOrthogonal:
                    ...
                    v = (
                        vprime
                        - np.dot(vprime, theta) * theta / np.linalg.norm(theta) ** 2
                    )
                ...
                self.Sigma_delta = (
                    random_matrix.T @ random_matrix + self.Sigma_delta * 10000
                )
            ...
            self.logger.info(
                f"Sigma_delta eigenvalues: {np.linalg.eigvals(self.Sigma_delta)}"
            )
    ...
    def generate_data(self, n, tau) -> DataSet:
        theta = np.random.default_rng().multivariate_normal(
            np.zeros(self.d), self.Sigma_theta, 1, method="cholesky"
        )[0]
        X = np.random.default_rng().multivariate_normal(
            np.zeros(self.d), self.Sigma_x, n, method="cholesky"
        )
        ...
        X_test = np.random.default_rng().multivariate_normal(
            np.zeros(self.d), self.Sigma_x, 10000, method="cholesky"
        )
        y_test = np.sign(
            X_test @ theta / np.sqrt(self.d) + tau * np.random.normal(0, 1, (10000,))
        )
...
class RandomCovariateDataModel(AbstractDataModel):
    def __init__(
        self, d, logger, source_pickle_path="../", delete_existing=False
    ) -> None:
        self.model_type = DataModelType.RandomCovariate
        super().__init__(d, logger, source_pickle_path, delete_existing)

        raise NotImplementedError(
            "RandomCovariateDataModel is only partially implemented, do not use it yet."
        )
        ...
        if min_eigval < 0:
            raise Exception(
                "Sigma_x is not positive definite; min eigval: ", min_eigval
            )
    ...
    def generate_data(self, n, tau) -> DataSet:
        the = np.random.default_rng().multivariate_normal(
            np.zeros(self.d), self.Sigma_theta, method="cholesky"
        )
        ...
        y_test = np.sign(
            X_test @ the / np.sqrt(self.d) + tau * np.random.normal(0, 1, (10000,))
        )
        return DataSet(X, y, X_test, y_test, the)
