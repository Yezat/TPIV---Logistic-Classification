...
def A_hat_func(m: float, q: float, sigma: float, A: float, N: float, rho_w_star: float, alpha: float, epsilon: float, tau:float, lam: float, int_lims: float = 20.0):
    ...
    def integrand(xi, y):
        ...
        if arg <= 0:
            ...
            second = sigma * np.exp(-arg) / ((1 + np.exp(-arg)) * cosh)
        epsilon_term = (first + second) * epsilon / np.sqrt(N)

        g_prime = 1/(1+np.exp(arg))
        g_prime *= 1.0 / (N**0.5)
        g_prime *= epsilon

        return z_0 * g_prime * gaussian(xi)
...
def N_hat_func(m: float, q: float, sigma: float, A: float, N: float, rho_w_star: float, alpha: float, epsilon: float, tau:float, lam: float, int_lims: float = 20.0):
    def integrand(xi, y):
        ...
        if arg <= 0:
            ...
            second = sigma * np.exp(-arg) / ((1 + np.exp(-arg)) * cosh)
        epsilon_term = (first + second) * epsilon * (-1/2) * A / (N**1.5)
...
if __name__ == "__main__":
    ...
    # start = time.time()
    # m,q,sigma,A,N, sigma_hat, q_hat, m_hat,A_hat,N_hat = fixed_point_finder(logging, kitchen_model,rho_w_star=1,alpha=n/d,epsilon=epsilon,tau=tau,lam=lam,abs_tol=TOL_FPE,min_iter=MIN_ITER_FPE,max_iter=MAX_ITER_FPE,blend_fpe=BLEND_FPE,int_lims=INT_LIMS,initial_condition=INITIAL_CONDITION)
    # print("m: ", m)
    # print("q: ", q)
    # print("sigma: ", sigma)
    ...
    # m: 56.518719365064044, q: 1616.0323865527955, sigma: 29.857262141808185, A: 43.21647944276486, N: 43.21647944276486
    # 2023-10-12 09:02:52,301 - INFO - m_hat: 0.49818856124476696, q_hat: 0.01694247163109377, sigma_hat: 0.017466812969044263,
    m = 56.518719365064044
    q = 1616.0323865527955
    sigma = 29.857262141808185
    A = 43.21647944276486
    N = 43.21647944276486
    m_hat = 0.49818856124476696
    q_hat = 0.01694247163109377
    sigma_hat = 0.017466812969044263

    a_hat = A_hat_func(m,q,sigma,A,N,kitchen_model.rho,alpha,epsilon,tau,lam,INT_LIMS)
    ...
    def integrand(xi, y):
        e = m * m / (kitchen_model.rho * q)
        w_0 = np.sqrt(kitchen_model.rho*e) * xi
        V_0 = kitchen_model.rho * (1-e)
    ...
    A_hat =  alpha * (Iplus + Iminus)
    print("A_hat: ", A_hat)

    # for xi in np.linspace(-1.1,1.1,100):
    #     print("xi: ", xi)
